...
$(document).ready(function(data){
	...
	socket.on('ResponseAlerts',function(data){
		...
		for(var i=0;i<lstAlerts.length;i++){
			$('#AlertsTable').append("<tbody><tr><td>"+lstAlerts[i].ALERTID+"</td><td>"+lstAlerts[i].JOURNEYID+"</td><td>"+lstAlerts[i].ALERTTYPE+"</td><td>"+lstAlerts[i].ALERTDESCRIPTION+"</td><td>"+lstAlerts[i].TRUCKID+"</td><td>"+lstAlerts[i].ALERTTIME.getHours()+"</td></tr></tbody>");
		}
