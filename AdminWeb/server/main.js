	...
	// });
      ...
      socket.on('AppNewUserRequest',function(data){
	  		console.log('AppNewUserRequest');
	    		connection.query('INSERT INTO person_temp (PERSONCIRUC,PERSONNAME,PERSONLASTNAME,PERSONPHONE,PERSONADDRESS,PERSONROLE) VALUES (?,?,?,?,?,?)',[data.ci,data.name,data.lastName,data.phone,data.address,data.role],function(err, rows, fields) {
		        	if(err){
	  ...
	  socket.on('RequestAlerts',function(data){
		  connection.query("select A.ALERTID, A.JOURNEYID, A.ALERTTYPE, A.ALERTDESCRIPTION, J.TRUCKID, date_format(A.ALERTTIME, '%d/%m/%Y %T' ) from alert A, journey J WHERE A.JOURNEYID=J.JOURNEYID;",function(error, result){
				if(error){
					throw error;
				}else{
